...
from models import (
    User, Student, Course, Teacher, Group, Lesson, Payment
)
...
def verify_user_login(conn: sqlite3.Connection, email: str, password: str) -> Optional[User]:
    row = conn.execute(
        "SELECT id, full_name, email, phone, password_hash, role, status, created_at FROM users WHERE email = ?",
        (email,),
    ).fetchone()
    if row and check_password_hash(row[4], password):
        return User(
            id=row[0], full_name=row[1], email=row[2], phone=row[3],
            role=row[5], status=row[6], created_at=row[7],
        )
    return None


def get_teacher_id_for_user(conn: sqlite3.Connection, user_id: int) -> Optional[int]:
    row = conn.execute(
        "SELECT id FROM teachers WHERE user_id = ? AND status = 'active'",
        (user_id,),
    ).fetchone()
...
def get_students(conn: sqlite3.Connection, query: str = "", sort_mode: str = "latest", status_filter: str = "all") -> List[Tuple]:
    sql = "SELECT id, full_name as display_name, email, phone, guardian_name, guardian_phone, status, notes, created_at FROM students WHERE 1=1"
    params = []
...
def add_student(conn: sqlite3.Connection, student: Student) -> int:
    ...
    cur.execute(
        """
        INSERT INTO students (full_name, email, phone, guardian_name, guardian_phone, status, notes)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (
            student.full_name,
...
def update_student(conn: sqlite3.Connection, student_id: int, student: Student) -> None:
    conn.execute(
        """
        UPDATE students
        SET full_name = ?, email = ?, phone = ?, guardian_name = ?, guardian_phone = ?, status = ?, notes = ?
        WHERE id = ?
        """,
        (
            student.full_name,
...
def get_student_detail(conn: sqlite3.Connection, student_id: int):
    student = conn.execute(
        "SELECT id, full_name, email, phone, guardian_name, guardian_phone, status, notes, created_at FROM students WHERE id = ?",
        (student_id,)
    ).fetchone()
    ...
    groups = conn.execute(
        """
        SELECT g.id, g.name, c.title, gs.status, gs.joined_at
    ...
    return student, groups, payments
...
def get_courses(conn: sqlite3.Connection, query: str = "") -> List[Tuple]:
    if query:
        return conn.execute(
            "SELECT id, title as display_name, description, category, default_fee, status FROM courses WHERE title LIKE ? OR category LIKE ? ORDER BY id DESC",
            (f"%{query}%", f"%{query}%"),
        ).fetchall()
    return conn.execute("SELECT id, title as display_name, description, category, default_fee, status FROM courses ORDER BY id DESC").fetchall()


def add_course(conn: sqlite3.Connection, course: Course) -> int:
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO courses (title, description, category, default_fee, status) VALUES (?, ?, ?, ?, ?)",
        (course.title or course.name, course.description, course.category, course.default_fee, course.status or "active")
    )
    return cur.lastrowid
...
def get_groups(
    ...
    sql = """
        SELECT g.id, g.name, c.title AS course_name,
               COALESCE(t.full_name, 'Nicht zugewiesen') AS teacher_name,
               g.capacity,
    ...
    if query:
        sql += " AND (g.name LIKE ? OR c.title LIKE ? OR t.full_name LIKE ?)"
        params.extend([f"%{query}%", f"%{query}%", f"%{query}%"])
...
def get_group_detail(conn: sqlite3.Connection, group_id: int):
    group = conn.execute(
        """
        SELECT g.id, g.name, c.title, COALESCE(t.full_name, 'Nicht zugewiesen'), g.capacity, g.start_date, g.end_date, g.schedule_description, g.status
        FROM groups g
        JOIN courses c ON g.course_id = c.id
        LEFT JOIN teachers t ON g.teacher_id = t.id
        WHERE g.id = ?
    ...
    members = conn.execute(
        """
        SELECT s.id, s.full_name, s.email, s.phone, gs.joined_at, gs.status
        FROM group_students gs
...
def get_lessons(
    ...
    sql = """
        SELECT l.id, g.name AS group_name,
               COALESCE(t.full_name, 'Nicht zugewiesen') AS teacher_name,
    ...
    if teacher_id is not None:
        sql += " AND COALESCE(l.teacher_id, g.teacher_id) = ?"
        params.append(teacher_id)
...
def get_lesson_attendance(conn: sqlite3.Connection, lesson_id: int):
    lesson = conn.execute(
        """
        SELECT l.id, g.name, COALESCE(t.full_name, 'Nicht zugewiesen'), l.starts_at, l.ends_at, l.room_label, l.topic, l.status, l.group_id
        FROM lessons l
        JOIN groups g ON l.group_id = g.id
        LEFT JOIN teachers t ON l.teacher_id = t.id
        WHERE l.id = ?
    ...
    records = conn.execute(
        """
        SELECT s.id, s.full_name, COALESCE(a.status, 'unmarked'), COALESCE(a.note, '')
        FROM group_students gs
        JOIN students s ON gs.student_id = s.id
...
def get_payments(conn: sqlite3.Connection, status_filter: str = "all") -> List[Tuple]:
    ...
    sql = """
        SELECT p.id, s.full_name as student_name, COALESCE(g.name, 'N/A') as group_name,
...
def record_payment_receipt(
    ...
    payment = conn.execute(
        "SELECT amount_due, amount_paid FROM payments WHERE id = ?",
        (payment_id,),
    ).fetchone()
    if not payment or paid_amount <= 0:
        return False

    remaining = payment[0] - payment[1]
    if remaining <= 0 or paid_amount > remaining:
        return False
