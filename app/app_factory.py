...
APP_DIR = Path(sys._MEIPASS) / "app" if getattr(sys, "frozen", False) else Path(__file__).resolve().parent
...
def create_app(config=None):
    ...
    if app.config["DEMO_MODE"] and not app.config["SECRET_KEY"]:
        app.config["SECRET_KEY"] = "local-demo-only-change-me"
    if not app.config["SECRET_KEY"]:
        raise ValueError("CRM_SECRET_KEY is required outside demo mode")
    ...
    database.init_db(app.config["DB_PATH"], seed_demo=app.config["DEMO_MODE"])
    register_routes(app)
